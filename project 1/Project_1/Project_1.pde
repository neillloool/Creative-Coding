...
final int minMove = 5;     // Minimum travel for a new point
...
a =random (50,100);
  ...
  updateGeometry();
  fill(r,g,b,a); //trippy shifting colors for lines
  for (int i = 0; i < nGestures; i++) {
...
void clearGestures() {
  ...
  //open processing  music player by cynthia nguyen
  //http://www.openprocessing.org/sketch/47667
  ...
  
{
 ...
 player=minim.loadFile("music.wav");//locate file in data
 player.play();//when opened play this
 player.loop(); // added this looping function myself
}
  {
   if (key== ' '){// click to spacebar stop music
  player.close();
  minim.stop();
